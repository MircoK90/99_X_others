...
def setup_llm() -> BaseChatModel:
    """Initialize local Ollama LLM — no API key needed."""
    ...
    llm = ChatOllama(
        ...
        temperature=0,
        num_ctx=4096,
        num_gpu=99,   # All layers into VRAM
    )
    logger.info(f"✓ LLM: Ollama / {llm.model}")
    return llm
...
def run_chat_pattern(llm: BaseChatModel) -> None:
    """Run Pattern 4: Interactive single-question chat."""
    print("\n" + "="*70)
    print("PATTERN 4: Interactive Chat")
    print("="*70)

    # System prompt für Mathe-Aufgaben
...
Examples:
  ...
  python -m src.main --pattern loop       # Run loop pattern only
    ...
    parser.add_argument(
        '--pattern',
        '-p',
        choices=['linear', 'conditional', 'loop', 'human', 'chat', 'all'],
        default='all',
